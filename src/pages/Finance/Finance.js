...
const Finance = () => {
  ...
  const getAllFeeAmountData = async () => {
    try {
      const { data } = await axios.get(
        "https://gcecbackend.onrender.com/api/v1/fee/all"
      );
      setFeeData(data);
    } catch (error) {
      console.log(error);
  ...
  const handleSaveData = async () => {
    ...
    try {
      const response = await axios.put(
        `https://gcecbackend.onrender.com/api/v1/student/updateFinance/${selectedStudent.slug}`,
        {
          financeProperties: [
            {
  ...
  const handleDeleteData = async () => {
    try {
      await axios.delete(
        `https://gcecbackend.onrender.com/api/v1/student/deleteFinance/${selectedStudent.slug}`,
        {
          data: {
            year: selectedAcademics,
            month: selectedMonth,
